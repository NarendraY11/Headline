...
create table if not exists public.study_plans (
  ...
  constraint study_plans_size check (pg_column_size(plan) <= 131072)
);
...
create index if not exists idx_missions_user_status      on public.study_missions(user_id, status);
create index if not exists idx_missions_plan             on public.study_missions(plan_id);
create index if not exists idx_missions_attempt          on public.study_missions(completed_attempt_id);
...
begin
  ...
  if new.plan_id is not null and not exists (
       select 1 from public.study_plans p where p.id = new.plan_id and p.user_id = new.user_id) then
  ...
  if new.completed_attempt_id is not null and not exists (
       select 1 from public.attempts a where a.id = new.completed_attempt_id and a.user_id = new.user_id) then
...
-- Feature flag ships dark (false). isFeatureEnabled fails open, so the key
-- must exist and be false. Only set when absent (never clobber an operator).
