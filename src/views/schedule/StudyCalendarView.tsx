// AI Study Scheduler — Flight Schedule calendar (Phase M6 + Phase 1 UX)
//
// Three views: Monthly | Weekly | Agenda
// Day Panel: click any day → full mission list + Start CTA
...
export default function StudyCalendarView() {
  ...
  // Track once per mount (not on remount — session-level)
  const [tracked, setTracked] = useState(false);
  useEffect(() => {
    if (schedulerEnabled && !tracked) {
      trackCalendarOpened();
      setTracked(true);
    }
  ...
  // Reactive today — updates after midnight without page reload.
  ...
  // Redirect when flag OFF — all hooks already called above.
  ...
  return (
    <div className="relative min-h-screen pb-24">
      ...
      <div className="relative z-10 max-w-[1100px] mx-auto px-4 pt-8 pb-6">
        ...
        <div className="mb-4">
          <div className="flex items-end justify-between gap-3 flex-wrap">
            ...
            {/* View toggle */}
            <div className="flex items-center gap-1 bg-bg-2 border border-rule rounded-xl p-1" role="group" aria-label="Calendar view">
        ...
        {/* ── Exam countdown — prominent, directly below header ── */}
        <ExamCountdown className="mb-4 w-full" />

        {/* ── Nav bar (monthly + weekly only) ── */}
        {view !== "agenda" && (
        ...
        {/* ── Main content + optional day panel ── */}
        {!isEmpty && (
