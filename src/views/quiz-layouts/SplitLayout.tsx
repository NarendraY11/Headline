...
export default function SplitLayout({
  ...
  return (
    <div className="flex flex-col h-screen h-[100dvh] bg-bg">
      ...
      {/* Progress Dot Bar Below Header */}
      <div className="flex items-center justify-center gap-2 w-full shrink-0 overflow-x-auto px-4 py-3 bg-bg border-b border-rule/50">
        {questions.map((q, idx) => {
      ...
      <main className="flex-1 w-full flex flex-col md:flex-row min-h-0 relative z-10 transition-all duration-300">
        
        {/* Left Pane (Scrollable Question Area) */}
        <div className="flex-1 w-full md:w-1/2 overflow-y-auto bg-bg flex flex-col relative z-20">
          <div className="w-full max-w-[640px] mx-auto px-4 sm:px-8 lg:px-12 pt-8 pb-24 flex-1 flex flex-col">
              ...
              >
                ...
                >
                  <span className="font-mono text-[10px] md:text-[11px] uppercase tracking-widest text-muted-2 mb-4 flex items-center gap-2">
                    <span className="text-signal text-[12px] leading-none mb-[2px]">♦</span> Q {currentIndex + 1} OF {totalQuestions}
                  </span>
                ...
                {currentQ.diagramCaption && (
                  <div className="md:hidden w-full mt-2 mb-8 bg-ink rounded-xl overflow-hidden relative border border-rule/20">
                     ...
                     <div className="p-4" style={{ '--ink': '#f2f1f0', '--paper': '#1c1b1a', '--white': 'rgba(255,255,255,0.05)', '--muted': 'rgba(255,255,255,0.4)', '--rule': 'rgba(255,255,255,0.1)' } as React.CSSProperties}>
                     ...
                     currentQ.choices.map((choice, i) => {
                       ...
                       return (
                         <motion.div
                           ...
                           className={`p-4 md:p-5 rounded-xl border flex items-center gap-4 md:gap-5 outline-none focus-visible:ring-2 focus-visible:ring-sky/60 focus-visible:border-sky-soft transition-shadow ${containerClass}`}
                         >
                ...
                <AnimatePresence>
                  {((isSubmittedPractice && mode === "practice") || (isRevealedViva && mode === "viva")) && (
                    <motion.div 
                      ...
                      className="mb-8 border-l-4 border-mint flex flex-col xl:flex-row shadow-sm bg-panel rounded-r-xl overflow-hidden overflow-y-hidden"
                    >
                       ...
                       <div className="bg-paper p-6 flex-1 border-t xl:border-t-0 xl:border-l border-rule flex flex-col h-auto">
                         ...
                         {!aiExplanations[currentQ.id] && !isAiLoading ? (
                           ...
                           <motion.div 
                             ...
                             className="mt-2 font-sans text-[13px] text-ink-2 leading-relaxed whitespace-pre-wrap"
                           >
          ...
          {/* Left Pane Sticky Bottom Nav */}
          <div className="sticky bottom-0 bg-bg mt-auto z-30 flex flex-col w-full">
            {/* Mobile Action Bar */}
            <div className="md:hidden pb-[calc(1rem+var(--sab))] pt-3 border-t border-rule px-4 flex items-center gap-3">
               <button 
                 onClick={() => toggleBookmark(currentQ)}
            ...
            {/* Desktop Action Bar */}
            <div className="hidden md:flex pb-[calc(1.5rem+var(--sab))] pt-4 border-t border-rule items-center justify-between px-4 sm:px-8 lg:px-12 w-full">
              <Button 
                variant="ghost" 
                onClick={handlePrev}
        ...
        {/* Right Pane (Diagram Dark Mode) */}
        <div className="hidden md:flex flex-col w-1/2 bg-ink h-full relative z-10 border-l border-rule/20 justify-between">
          
          {/* Right Pane Header: Mono caption */}
          <div className="flex justify-between items-center px-6 h-12 border-b border-white/10 shrink-0">
             <span className="font-mono text-[10px] uppercase tracking-widest text-[#f2f1f0]/50">
                FIG. {currentQ.ata.split('·')[0].trim().replace(' ', '-')} · {currentQ.diagramCaption}
             </span>
             <span className="font-mono text-[10px] uppercase tracking-widest text-[#f2f1f0]/30 hidden lg:inline-block">
                PLATE 27-04 · REV 41
             </span>
          </div>
          ...
          <div className="flex-1 flex items-center justify-center p-8 overflow-hidden relative">
             ...
             {/* Render actual system diagram inside dark mode container */}
             <div className="w-full max-w-[500px]" style={{ '--ink': '#f2f1f0', '--paper': '#1c1b1a', '--white': 'rgba(255,255,255,0.05)', '--muted': 'rgba(255,255,255,0.4)', '--rule': 'rgba(255,255,255,0.1)' } as React.CSSProperties}>
                 <FlightControlsDiagram />
             </div>
          </div>
          
          {/* Bottom SPEC STRIP */}
          <div className="h-16 shrink-0 border-t border-white/10 flex items-stretch">
             <div className="flex-1 flex flex-col justify-center px-6 border-r border-white/10">
               <span className="font-mono text-[9px] text-[#f2f1f0]/40 uppercase tracking-widest">PRIM</span>
               <span className="font-serif text-[#f2f1f0] text-xl leading-none mt-1">2</span>
             </div>
             <div className="flex-1 flex flex-col justify-center px-6 border-r border-white/10">
               <span className="font-mono text-[9px] text-[#f2f1f0]/40 uppercase tracking-widest">SEC</span>
               <span className="font-serif text-[#f2f1f0] text-xl leading-none mt-1">3</span>
             </div>
             <div className="flex-[2] flex flex-col justify-center px-6">
               <span className="font-mono text-[9px] text-[#f2f1f0]/40 uppercase tracking-widest">LAW</span>
               <span className="font-serif text-[#f2f1f0] text-xl leading-none mt-1">Normal</span>
             </div>
          </div>
