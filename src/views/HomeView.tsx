...
function InteractiveSampleQuestion() {
  ...
  if (currentIndex >= 5) {
    return (
      <div className="bg-paper border border-rule rounded-2xl p-5 sm:p-6 md:p-8 shadow-sm text-center relative overflow-hidden w-full max-w-[90vw] sm:max-w-sm md:max-w-md mx-auto md:mx-0 shrink-0 h-[430px] flex flex-col justify-center items-center">
        ...
        <h3 className="font-serif text-[24px] text-ink mb-3">Ready to start?</h3>
        <p className="font-sans text-[15px] text-ink-2 mb-8 max-w-[250px]">
          You've completed the sample questions. Create a free account to track your progress and access the full question bank.
        </p>
        <Link to="/modules">
          <Button variant="primary" className="h-[48px] px-8 rounded-full bg-ink text-bg">Try the full bank <MoveRight size={16} className="ml-2" /></Button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <motion.div 
      className="bg-paper border border-rule rounded-2xl py-5 px-6 shadow-sm text-left relative overflow-hidden w-full max-w-[90vw] sm:max-w-sm md:max-w-md mx-auto md:mx-0 shrink-0 min-h-[430px] flex flex-col"
    ...
    >
      ...
      <div className="absolute top-6 right-2 transform rotate-12 opacity-30 font-mono text-[10px] tracking-widest pointer-events-none select-none text-ink">
        {currentQ.ata.split(" ")[1] || "27"}-04 REV 1
      </div>
      ...
      <div className="font-mono text-[9px] text-muted-2 tracking-widest uppercase mb-1.5">QUESTION</div>
      <p className="font-serif text-[18px] md:text-[20px] leading-snug text-ink mb-3 tracking-tight min-h-[60px]">
        {currentQ.prompt}
      </p>
        ...
        {currentQ.choices.map((o) => {
          ...
          return (
            <button 
              ...
              className={`w-full text-left flex items-center py-[10px] px-[14px] border rounded-[12px] transition-all ${borderClass}`}
            >
              <span className={`font-mono text-[10px] shrink-0 w-6 h-6 flex items-center justify-center rounded-full border ${iconClass}`}>
                {iconContent}
              </span>
              <span className="font-sans text-[13px] font-medium ml-3 sm:ml-4 leading-snug">
                {o.label}
              </span>
            </button>
          )
      ...
      <AnimatePresence>
      {submitted && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mb-6 font-sans text-[13px] text-ink-2 bg-rule/30 p-3 rounded-lg border border-rule overflow-hidden">
          <span className="font-semibold text-ink">Explanation:</span> {currentQ.explanation.split('.')[0]}.
        </motion.div>
      )}
      </AnimatePresence>

      <div className="flex items-center justify-between mt-auto border-t border-transparent relative">
        <div className="flex gap-2 sm:gap-3 font-mono text-[8px] sm:text-[9px] text-muted-2 tracking-[0.1em] sm:tracking-[0.2em] uppercase">
          <button onClick={() => handleToolbarClick("BOOKMARK")} className="hover:text-ink transition-colors outline-none focus-visible:ring-2 focus-visible:ring-sky/60">BOOKMARK</button>
          <span>·</span>
          <button onClick={() => handleToolbarClick("NOTE")} className="hover:text-ink transition-colors outline-none focus-visible:ring-2 focus-visible:ring-sky/60">NOTE</button>
          <span>·</span>
          <button onClick={() => handleToolbarClick("SKIP")} className="hover:text-ink transition-colors outline-none focus-visible:ring-2 focus-visible:ring-sky/60">SKIP</button>
        </div>
        
        <AnimatePresence>
        {showTooltip && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="absolute bottom-14 left-0 font-sans text-xs bg-ink text-bg px-3 py-1.5 rounded shadow-lg whitespace-nowrap z-20">
            {tooltipMessage}
