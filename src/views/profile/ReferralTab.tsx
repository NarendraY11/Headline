// UX-Nav Phase 2C: Referral tab — the old /referral page re-homed inside the
// Profile workspace. Business logic (Supabase stats, copy, ledger) is unchanged
// from ReferralView; only the page-level wrapper + useDocumentMeta were dropped
// since the Profile shell owns layout + meta now.

import { Check, Clock, Copy, Gift, ShieldCheck, Users } from "lucide-react";
...
export default function ReferralTab() {
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    trackEvent("profile_referral_copied", { metadata: { code: referralCode } });
    ...
    <div className="space-y-8 font-sans max-w-4xl">
      ...
      <Card className="p-6 border border-rule-strong bg-paper rounded-2xl shadow-sm space-y-4">
        <div className="space-y-1">
          <h3 className="font-serif text-lg font-bold text-ink">Your Pilot Referral Dispatch Link</h3>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
          ...
          <Button
            ...
            onClick={handleCopy}
            className={`sm:w-auto h-11 px-6 rounded-lg text-xs font-mono uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer transition-all ${copied ? "bg-emerald-50 text-emerald-600 border border-emerald-200" : "bg-ink hover:bg-ink-2 text-bg"}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="space-y-2 p-5 bg-paper border border-rule rounded-xl">
          <div className="w-8 h-8 rounded-full bg-navy/10 text-navy flex items-center justify-center font-bold font-mono text-xs">1</div>
          <h4 className="font-serif text-sm font-bold text-ink">Friend boards the cockpit</h4>
          <p className="text-muted text-xs leading-relaxed">Your friend registers an account on Heading using your unique link in browser. The referral token is saved instantly.</p>
        </div>
        <div className="space-y-2 p-5 bg-paper border border-rule rounded-xl">
          <div className="w-8 h-8 rounded-full bg-navy/10 text-navy flex items-center justify-center font-bold font-mono text-xs">2</div>
          <h4 className="font-serif text-sm font-bold text-ink">Friend upgrades to Pro</h4>
          <p className="text-muted text-xs leading-relaxed">When they feel ready to unlock C1 syllabus questions, premium calculators, and AI ground instructor help, they subscribe to Pro.</p>
        </div>
        <div className="space-y-2 p-5 bg-paper border border-rule rounded-xl">
          <div className="w-8 h-8 rounded-full bg-emerald-500/10 text-emerald-600 flex items-center justify-center font-bold font-mono text-xs">3</div>
          <h4 className="font-serif text-sm font-bold text-ink">Double Rewards Granted</h4>
          <p className="text-muted text-xs leading-relaxed">Our payment verification webhook immediately triggers. Adding +30 days on your account and an extra +30 days on theirs!</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="p-10 text-center max-w-sm mx-auto space-y-2.5">
              ...
              <p className="text-muted text-xs leading-relaxed">When fellow candidates register on your shared channel, their flight logs will accumulate inside this table immediately.</p>
            </div>
          ) : (
            <table className="min-w-full divide-y divide-rule text-xs font-sans">
              ...
              <tbody className="divide-y divide-rule bg-paper">
                {referrals.map((item) => {
                  ...
                  const maskedEmail = referredEmail ? referredEmail.replace(/(.{2})(.*)(@.*)/, "$1***$3") : "Candidate Pilot";
                  const isCompleted = item.status === "completed" || item.reward_granted;
                  return (
                    <tr key={item.id} className="hover:bg-bg-2/30">
                      <td className="px-5 py-3 font-semibold text-ink">{item.referred?.display_name || maskedEmail}</td>
                      <td className="px-5 py-3 text-muted">{new Date(item.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</td>
                      <td className="px-5 py-3">
                        {isCompleted ? (
                          <span className="inline-flex items-center gap-1 text-[#059669] bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded font-mono text-[9px] font-bold uppercase"><ShieldCheck size={11} /> Upgraded PRO</span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-slate-500 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded font-mono text-[9px] font-bold uppercase"><Clock size={11} /> Registered Free</span>
                        )}
                      </td>
                      <td className="px-5 py-3 text-right text-muted font-mono text-[10.5px]">
                        {isCompleted ? <span className="text-emerald-600 font-bold">+30 Days Active Pro</span> : <span className="text-slate-400">Claims on friend checkout</span>}
                      </td>
                    </tr>
                  );
                })}
