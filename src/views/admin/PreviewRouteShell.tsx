...
function PreviewRouteShellInner({
  ...
  return (
    <div className={embedded ? "h-full bg-bg" : "mx-auto max-w-6xl"}>
      <header className={embedded ? "border-b border-rule bg-paper px-4 py-3" : "mb-5 rounded-xl border border-rule-strong bg-paper px-5 py-4"}>
        <div className="flex flex-wrap items-center gap-2 text-[10px] font-mono uppercase tracking-[0.22em] text-muted-2">
          <Route size={12} className="text-ink" aria-hidden="true" />
          <span>Route Preview</span>
          <span className="text-rule-strong" aria-hidden="true">/</span>
          <span>{feature.key}</span>
        </div>
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <h2 className="font-serif text-lg text-ink">{feature.title}</h2>
          <span
            className="rounded-full border border-rule px-2 py-1 font-mono text-[10px] uppercase tracking-widest text-muted"
            aria-label={`Flag is currently ${enabled ? "enabled" : "disabled"}`}
          >
        ...
        <div className="mt-3 flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-muted-2">
          ...
          <span aria-label={`Preview route path: ${routePath}`}>{routePath}</span>
        </div>
      </header>
      ...
      {!enabled ? (
        <div
          role="status"
          aria-label={`${feature.title} route preview is unavailable because the flag is disabled`}
          className="flex min-h-[320px] flex-col items-center justify-center rounded-b-xl border-t border-rule bg-bg-2/50 px-6 py-10 text-center"
        >
          <ShieldOff size={28} className="text-muted-2" aria-hidden="true" />
          <h3 className="mt-4 font-serif text-xl text-ink">Route hidden while flag is off</h3>
